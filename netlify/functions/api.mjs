...
export default async (req) => {
  ...
  if(p==='guidance' && req.method==='GET'){
    ...
    const animals=aid
      ? await db.sql`SELECT id,name,status FROM animals WHERE id=${aid}`
      : me.role==='admin'
        ? await db.sql`SELECT id,name,status FROM animals ORDER BY name`
        : await db.sql`SELECT a.id,a.name,a.status FROM animals a LEFT JOIN animal_access aa ON aa.animal_id=a.id AND aa.user_id=${me.id} WHERE a.owner_id=${me.id} OR aa.user_id IS NOT NULL ORDER BY a.name`;
  ...
  if(p==='animals'){
    ...
    if(req.method==='POST' && ['admin','owner'].includes(me.effective_role)){
      ...
      await db.sql`INSERT INTO animals(id,name,species,breed,owner_id) VALUES(${aid},${String(b.name||'').trim()},${b.species||''},${b.breed||''},${ownerId})`;
  ...
  if(p==='food'){
    ...
    if(req.method==='POST'){
      if(!['admin','keeper','owner'].includes(me.effective_role))return json({error:'Только владелец, кипер или администратор'},403); const b=await parse(req); if(!await allowedAnimal(me,b.animal_id))return json({error:'Нет доступа к животному'},403); const fid=id(); const products=JSON.stringify(Array.isArray(b.products)?b.products:[]); await db.sql`INSERT INTO food_logs(id,animal_id,author_id,logged_at,meal,offered,eaten,not_eaten,appetite,note,products) VALUES(${fid},${b.animal_id},${me.id},${b.logged_at||new Date().toISOString()},${b.meal||''},${b.offered||''},${b.eaten||''},${b.not_eaten||''},${b.appetite||null},${b.note||''},${products}::jsonb)`;return json({ok:true,id:fid},201);
    }
    if(req.method==='DELETE'){
      if(!['admin','keeper','owner'].includes(me.effective_role))return json({error:'Нет прав на удаление'},403); const b=await parse(req); const rows=await db.sql`SELECT animal_id FROM food_logs WHERE id=${b.id}`; if(!rows.length)return json({error:'Запись не найдена'},404); if(!await allowedAnimal(me,rows[0].animal_id))return json({error:'Нет доступа'},403); await db.sql`DELETE FROM food_logs WHERE id=${b.id}`; return json({ok:true});
    }
  }
